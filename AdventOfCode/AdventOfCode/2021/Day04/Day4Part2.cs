...
namespace AdventOfCode._2021.Day04
{
    public class Day4Part2
    {
        ...
        private readonly List<(List<List<int>> board, bool[,] isVisited, bool hasWon)> _bingoBoards = new List<(List<List<int>> board, bool[,] isVisited, bool hasWon)>();
        ...
        private void Day4()
        {
            ...
            foreach (var current in _numbers)
            {
                ...
                {
                    (List<List<int>> board, bool[,] isVisited, bool hasWon) = _bingoBoards[i];
                    for (int j = 0; j < 5; j++)
                    {
                        List<int> row = board[j];
                    ...
                    if (!hasWon && IsWinner(isVisited))
                    {
                        _bingoBoards[i] = (board, isVisited, true);
                        int sum = 0;
                        for (int j = 0; j < 5; j++)
                        {
                            for (int k = 0; k < 5; k++)
        ...
        private void ReadData()
        {
            ...
            for (int i = 0; i < boards.Count; i += 5)
            {
                ...
                _bingoBoards.Add((temp, isVisited, false));
            }
        }
